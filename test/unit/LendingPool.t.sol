...
contract LendingPoolTest is Test {
    ...
    function test_Withdraw_RevertInsufficientBalance() public {
        _depositLiquidity(100 ether);

        vm.prank(lender);
        vm.expectRevert(LendingPool.InsufficientBalance.selector);
        pool.withdraw(101 ether);
    }
    ...
    function test_Repay_RevertNoDebt() public {
        borrowAsset.mint(borrower, 1 ether);
    ...
    function test_Repay_MoreThanDebtClearsDebt() public {
        _openBorrow(10 ether, 500 ether, 5_000 ether);
        ...
        assertEq(pool.currentDebt(borrower), 0);
        assertEq(pool.borrowIndex(borrower), 0);
    }

    function test_CurrentDebtZeroForUserWithoutBorrow() public view {
        assertEq(pool.currentDebt(borrower), 0);
    }
    ...
    function test_WithdrawCollateral_RevertZeroAmount() public {
        vm.prank(borrower);
        vm.expectRevert(LendingPool.InvalidAmount.selector);
        pool.withdrawCollateral(0);
    }

    function test_WithdrawCollateral_RevertInsufficientBalance() public {
        vm.prank(borrower);
        vm.expectRevert(LendingPool.InsufficientBalance.selector);
        pool.withdrawCollateral(1 ether);
    }

    function test_WithdrawCollateral_SucceedsWithoutDebt() public {
        vm.prank(borrower);
        pool.depositCollateral(2 ether);

        uint256 beforeBalance = collateral.balanceOf(borrower);
        vm.prank(borrower);
        pool.withdrawCollateral(1 ether);

        assertEq(collateral.balanceOf(borrower), beforeBalance + 1 ether);
        assertEq(pool.collateralBalance(borrower), 1 ether);
    }

    function test_WithdrawCollateral_RevertUnderCollateralized() public {
        _openBorrow(10 ether, 1_000 ether, 5_000 ether);
    ...
    function test_AvailableLiquidity() public {
        _depositLiquidity(1_000 ether);
        assertEq(pool.availableLiquidity(), 1_000 ether);
        ...
        assertEq(pool.availableLiquidity(), 900 ether);
    }

    function test_PauseBlocksBorrowAndUnpauseRestores() public {
        _depositLiquidity(5_000 ether);
        vm.prank(borrower);
        ...
        assertEq(pool.currentDebt(borrower), 100 ether);
    }
    ...
    function test_Liquidate_RevertZeroAmount() public {
        vm.prank(liquidator);
        vm.expectRevert(LendingPool.InvalidAmount.selector);
    ...
    function test_InitializeRevertInvalidAddress() public {
        LendingPool implementation = new LendingPool();
        ERC1967Proxy proxy = new ERC1967Proxy(address(implementation), "");
        LendingPool invalidPool = LendingPool(address(proxy));

        vm.expectRevert(LendingPool.InvalidAddress.selector);
        invalidPool.initialize(address(collateral), address(collateral), address(oracle), address(irm));
    }
    ...
    function test_ChainlinkAdapter_RevertStalePrice() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ChainlinkAdapter.StalePrice.selector, address(collateral), block.timestamp - 7200)
        );
    ...
    function testFuzz_BorrowWithinCollateralLimit(uint256 collateralAmount, uint256 borrowAmount) public {
        collateralAmount = bound(collateralAmount, 1 ether, 100 ether);
        uint256 maxBorrow = collateralAmount * 2_000 * 75 / 100;
        borrowAmount = bound(borrowAmount, 1, maxBorrow);

        _depositLiquidity(maxBorrow + 1 ether);
        vm.prank(borrower);
        pool.depositCollateral(collateralAmount);
        vm.prank(borrower);
        pool.borrow(borrowAmount);

        assertEq(pool.currentDebt(borrower), borrowAmount);
        assertGe(pool.getHealthFactor(borrower), 1e18);
    }
