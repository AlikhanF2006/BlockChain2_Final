...
contract YieldVaultTest is Test {
    ...
    function test_Withdraw_ToDifferentReceiver() public {
        ...
        vm.startPrank(user);
        vault.deposit(1_000 ether, user);
        vault.withdraw(250 ether, receiver, user);
        vm.stopPrank();

        assertEq(asset.balanceOf(receiver), 250 ether);
        assertEq(pool.deposits(address(vault)), 750 ether);
    }
    ...
    function test_RedeemRevertInsufficientShares() public {
        vm.prank(user);
        vm.expectRevert();
    ...
    function test_RedeemSuccess() public {
        vm.startPrank(user);
        uint256 shares = vault.deposit(500 ether, user);
        vault.redeem(shares / 2, user, user);
        vm.stopPrank();

        assertEq(vault.balanceOf(user), shares - shares / 2);
        assertEq(pool.deposits(address(vault)), 250 ether);
    }

    function test_UUPS_UpgradeAuthorized() public {
    ...
    function test_ERC4626_MaxWithdrawAndRedeem() public {
        vm.prank(user);
        vault.deposit(1_000 ether, user);

        assertEq(vault.maxWithdraw(user), 1_000 ether);
        assertEq(vault.maxRedeem(user), 1_000 ether);
    }

    function test_ERC4626_PreviewMintWithdrawRedeem() public {
        assertEq(vault.previewMint(100 ether), 100 ether);

        vm.prank(user);
        vault.deposit(1_000 ether, user);

        assertEq(vault.previewWithdraw(250 ether), 250 ether);
        assertEq(vault.previewRedeem(250 ether), 250 ether);
    }

    function test_Mint_ForwardsAssetsToLendingPool() public {
        vm.prank(user);
        uint256 assets = vault.mint(100 ether, user);

        assertEq(assets, 100 ether);
        assertEq(vault.balanceOf(user), 100 ether);
        assertEq(pool.deposits(address(vault)), 100 ether);
    }
    ...
    function testFuzz_PreviewDepositMatchesFirstDeposit(uint256 assets) public {
        assets = bound(assets, 1, 1e24);
        asset.mint(user, assets);
...
contract YieldVaultInvariantTest is Test {
    ...
    function invariant_TotalAssetsBackShares() public view {
