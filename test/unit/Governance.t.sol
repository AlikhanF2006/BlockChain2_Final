...
contract GovernanceTest is Test {
    ...
    address[] internal lifecycleTargets;
    uint256[] internal lifecycleValues;
    bytes[] internal lifecycleCalldatas;
    string internal lifecycleDescription;
    bytes32 internal lifecycleDescriptionHash;
    uint256 internal lifecycleProposalId;
    ...
    function test_FullGovernanceCycle() public {
        ...
        assertEq(uint256(governor.state(lifecycleProposalId)), uint256(IGovernor.ProposalState.Succeeded));

        _queueProposal();
        assertEq(uint256(governor.state(lifecycleProposalId)), uint256(IGovernor.ProposalState.Queued));

        _executeProposal();
        assertEq(uint256(governor.state(lifecycleProposalId)), uint256(IGovernor.ProposalState.Executed));
    ...
    function _createProposal() private {
        _storeClaimFeesProposal(1000e6);

        vm.prank(bob);
        lifecycleProposalId =
            governor.propose(lifecycleTargets, lifecycleValues, lifecycleCalldatas, lifecycleDescription);
    }

    function _advancePastVotingDelay() private {
        vm.roll(block.number + governor.votingDelay() + 1);
    }

    function _castVote() private {
        vm.prank(alice);
        governor.castVote(lifecycleProposalId, 1);
    }

    function _advancePastVotingPeriod() private {
        vm.roll(block.number + governor.votingPeriod() + 1);
    }

    function _queueProposal() private {
        governor.queue(lifecycleTargets, lifecycleValues, lifecycleCalldatas, lifecycleDescriptionHash);
    }

    function _executeProposal() private {
        vm.warp(block.timestamp + timelock.MIN_DELAY() + 1);
        governor.execute(lifecycleTargets, lifecycleValues, lifecycleCalldatas, lifecycleDescriptionHash);
    }

    function _storeClaimFeesProposal(uint256 amount) private {
        delete lifecycleTargets;
        delete lifecycleValues;
        delete lifecycleCalldatas;

        lifecycleTargets.push(address(treasury));
        lifecycleValues.push(0);
        lifecycleCalldatas.push(abi.encodeCall(Treasury.claimERC20, (address(usdc), recipient, amount)));
        lifecycleDescription = "Claim protocol USDC fees";
        lifecycleDescriptionHash = keccak256(bytes(lifecycleDescription));
    }
        ...
        returns (address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description)
    {
        ...
        values[0] = 0;
        calldatas[0] = abi.encodeCall(Treasury.claimERC20, (address(usdc), recipient, amount));
