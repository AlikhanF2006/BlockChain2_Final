...
contract GovTokenInvariantHandler is TestHelpers {
    GovToken public token;

    constructor(GovToken token_) {
        token = token_;
    }

    function transferFromDeployer(address to, uint256 amount) external {
...
contract GovTokenTest is TestHelpers {
    ...
    function test_PermitRevertExpiredDeadline() public {
        uint256 amount = 1_000 ether;
    ...
    function testFuzz_MintByAdmin(address to, uint256 amount) public {
        vm.assume(to != address(0));
        ...
        assertEq(token.balanceOf(to), to == deployer ? supplyBefore + amount : amount);
        assertEq(token.totalSupply(), supplyBefore + amount);
    }
}

contract GovTokenInvariantTest is TestHelpers {
    GovToken internal token;
    GovTokenInvariantHandler internal handler;

    function setUp() public {
        vm.prank(deployer);
        token = new GovToken();
        handler = new GovTokenInvariantHandler(token);
        targetContract(address(handler));
    }

    function invariant_TotalSupplyConservedAcrossTransfers() public view {
        assertEq(token.totalSupply(), token.INITIAL_SUPPLY());
    }
}
